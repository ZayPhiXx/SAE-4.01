...
[...]
	...
	\begin{itemize}
		...
		\item (L10) on indique qu'on écoute les requêtes TLS liées au blocs TLS sur toutes les adresses
	\end{itemize}
\end{enumerate}
...
\end{configbox}
\begin{itemize}
	\item (L1) On déclare le DNS qui va être utilisé (\ip{192.168.110.24}) et son hostname (bav4.local), spécifié lors de la création du certificat vu plus haut dans ce document
	\item (L2) On active le mode DNSOverTLS
\end{itemize}
...
\begin{enumerate}
	\item On va tenter d'utiliser \monosp{systemd-resolved}
	\item Si cela échoue, le système vérifie le fichier local \monosp{/etc/hosts}	
...
\begin{itemize}
	...
	\item (L30) Redemarrage de \monosp{systemd-resolved}
	\item (
\end{itemize}


Le script suivant est a esecuter sur la machine bastion, en ayant les fichiers \monosp{nsswitch.conf}, \monosp{resolve.conf}, \monosp{scriptDoTClient.sh}, a chaque connection, on effectuera en plus la commande
\cmd{su -c /tmp/scriptDoTClient.sh}
