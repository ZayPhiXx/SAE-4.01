...
\subsubsection{Haute disponibilité avec Keepalived avec le routeur}

Afin d’assurer la tolérance aux pannes et la continuité de service réseau, nous avons mis en place un mécanisme de \textit{haute disponibilité} (HA) entre deux routeurs à l’aide de l’outil \textbf{Keepalived}. Cette solution repose sur le protocole \textbf{VRRP} (Virtual Router Redundancy Protocol) qui permet de partager une IP virtuelle entre plusieurs machines redondants.

\paragraph{Principe de fonctionnement :}
Tout est dans la configuration d'un fichier; avec keepalived on choisit un \textbf{routeur actif (MASTER)} et un autre de secours (\textbf{BACKUP}). Le routeur actif gère l’IP virtuelle (VIP) utilisée par les clients comme passerelle. En cas de défaillance du MASTER (ex. perte d’interface, arrêt du service), le BACKUP avec la priorité qui suit, prend automatiquement le relais.
...
Une IP virtuelle commune est utilisée par interface comme passerelle pour les clients, par exemple :
\ip{192.168.20.1/24} pour la DMZ
...
Le fichier de configuration principal est \texttt{/etc/keepalived/keepalived.conf}. Voici un exemple de configuration
...
vrrp_instance VI_WAN {
    ...
    # Verifie que l'interface physique est bien UP
    track_interface {
        ens18
    }
    # Verifie que le pare-feu nftables est toujours charge
    track_script {
        check_nft
    }
    # Declenche des scripts selon les transitions d'etat VRRP
    notify_master "/etc/keepalived/scripts/notify_vrrp.sh MASTER ens18"
    notify_backup "/etc/keepalived/scripts/notify_vrrp.sh BACKUP ens18"
    notify_fault  "/etc/keepalived/scripts/notify_vrrp.sh FAULT ens18"
}
    \end{lstlisting}
\end{configbox}

Sur le BACKUP, seule la ligne \texttt{state MASTER} devient \texttt{state BACKUP}, et la priorité est réduite (par exemple \texttt{priority 90}).

\paragraph{Fonctionnalités avancées :}
Nous avons renforcé la solution en ajoutant :
\begin{itemize}
  \item Des scripts de notification lors des changements d’état (MASTER, BACKUP, FAULT)
  \item Une surveillance de l’état du pare-feu via un script personnalisé (\texttt{chk\_nft.sh})
  \item Des logs structurés dans \texttt{journalctl}, au format JSON, pour faciliter l’analyse ou l’intégration dans un SIEM
\end{itemize}
...
\paragraph{Limites possibles :}
\begin{itemize}
  \item Nécessite une bonne synchronisation des règles de pare-feu entre routeurs
  \item VRRP n'inclut pas de réplication d’état, uniquement de rôle
\end{itemize}
